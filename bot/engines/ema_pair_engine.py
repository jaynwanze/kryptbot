#!/usr/bin/env python3
# ────────────────────────────────────────────────────────────────
#  EMA TREND FOLLOWING LIVE ENGINE (1H Timeframe)
# ────────────────────────────────────────────────────────────────
"""
Live trading engine for EMA TREND FOLLOWING strategy on 1H timeframe.

Key Features:
- 200 EMA trend filter (only long above, only short below)
- EMA 21/50 crossover detection
- Optional HYBRID regime filter (toggle in config)
- Strong ADX requirement (configurable)
- Trails winners, cuts losses

Strategy:
- LONG: Price > 200 EMA + EMA21 crosses above EMA50 + ADX strong
- SHORT: Price < 200 EMA + EMA21 crosses below EMA50 + ADX strong
"""
...
from bot.helpers import (
    ...
    ema_long_signal,   # ← EMA signals
    ema_short_signal,  # ← EMA signals
    telegram,
    build_h1,
    update_h1,
    in_good_hours,
    ...
    append_csv,
    hours,
    precompute_regime,      # ← Regime filter
    regime_gate_fast,       # ← Regime filter
)
...
#  Global constants
# ────────────────────────────────────────────────────────────────
LOG_DIR = Path(getattr(ema_config, "LOG_DIR", "./logs"))
LOG_DIR.mkdir(parents=True, exist_ok=True)

# Cooldowns
COOLDOWN_DAYS_AFTER_SL = getattr(ema_config, "COOLDOWN_DAYS_AFTER_SL", 0.5)
COOLDOWN_DAYS_AFTER_TP = getattr(ema_config, "COOLDOWN_DAYS_AFTER_TP", 0.25)
...
GOOD_HOURS = _union_hours(ema_config.SESSION_WINDOWS)
...
PING_SEC = 20
TF = ema_config.INTERVAL  # "60" for 1H
...
async def kline_stream(pair: str, router: RiskRouter) -> None:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # PRE-COMPUTE REGIME FILTER (if enabled)
    # ═══════════════════════════════════════════════════════════════════════
    regime_enabled = getattr(ema_config, "REGIEME_FILTER_ENABLED", False)
    regime_series = None
    
    if regime_enabled:
        try:
            logging.info("[%s] Pre-computing regime filter...", pair)
            regime_series = precompute_regime(hist, ema_config)
            tradeable_pct = regime_series.sum() / len(regime_series) * 100
            logging.info("[%s] Regime filter: %.1f%% bars tradeable", pair, tradeable_pct)
        except Exception as e:
            logging.error("[%s] Regime filter failed: %s - DISABLING", pair, e)
        ...
        try:
            async with websockets.connect(
                ...
                async for raw in ws:
                    ...
                    if current_end > expected:
                        ...
                        for _ in range(MAX_RETRY):
                            try:
                                ...
                                # Re-compute regime for new bars
                                if regime_enabled:
                                    regime_series = precompute_regime(hist, ema_config)
                                break
                            except Exception as e:
                    ...
                    # Update regime series if enabled
                    if regime_enabled:
                        # Extend regime series with new bar
                        # For simplicity, re-compute last N bars (fast enough for 1H)
                        regime_series = precompute_regime(hist.tail(500), ema_config)

                    # Log drop stats every hour
                    if int(time.time()) % 3600 < TF_SEC:
                        logging.info("DROP_STATS %s %s", pair, drop_stats)
                        async with ema_config.GLOBAL_DROP_STATS_LOCK:
                            for key, val in drop_stats.items():
                                ema_config.GLOBAL_DROP_STATS[pair][key] = val

                    bar = hist.iloc[-1]
                    if bar[["atr", "atr30", "adx", "ema21", "ema50", "ema200"]].isna().any():
                        continue
                    ...
                    # 2) Proximity to HTF levels (relaxed for trend following)
                    if not near_htf_level(
                        bar,
                        htf_row,
                        max_atr=getattr(ema_config, "NEAR_HTF_MAX_ATR_MOM", 2.0),
                    ):
                        h1 = update_h1(h1, bar.name, float(bar.c))
                        htf_levels = update_htf_levels_new(htf_levels, bar)
                    ...
                    # 3) Session gate (24/7 for trends usually)
                    ...
                    # 4) H1 trend row (for compatibility)
                    try:
                    ...
                    min_adx = max(
                        min_adx, getattr(ema_config, "ADX_HARD_FLOOR", 50)
                    )
                    ...
                    if veto:
                        logging.info(
                            "[%s] No-trade (veto)  adx=%.1f  atr=%.5f",
                            pair, bar.adx, bar.atr
                        )
                        h1 = update_h1(h1, bar.name, float(bar.c))
                        htf_levels = update_htf_levels_new(htf_levels, bar)
                        continue
                    ...
                    stop_off = (
                        cushion * sl_base + ema_config.WICK_BUFFER * bar.atr
                    )
                    tp_dist = ema_config.RR_TARGET * stop_off

                    i = len(hist) - 1
                    header = "EMA TREND Engine (1H)"
                    ...
                    decision_log = {
                        ...
                        "price_vs_200ema": "above" if bar.c > bar.ema200 else "below",
                        "min_adx": min_adx,
                        "regime": "tradeable" if not regime_enabled or regime_gate_fast(bar.name, regime_series) else "choppy",
                    }
                    append_csv("decisions_ema.csv", decision_log, list(decision_log.keys()), log_dir=LOG_DIR)

                    # ═══════════════════════════════════════════════════════
                    # EMA TREND SIGNAL CHECKS
                    # ═══════════════════════════════════════════════════════
                    
                    # LONG: EMA 21 > 50, Price > 200 EMA
                    if ema_long_signal(hist, i, htf_row):
                        if router.has_open(pair):
                            logging.info("[%s] No-trade (already open)", pair)
                        else:
                            logging.info(
                                "[%s] LONG TREND signal  ema21=%.2f > ema50=%.2f | price=%.2f > ema200=%.2f | adx=%.1f",
                                pair,
                                bar.ema21,
                                bar.ema50,
                                bar.c,
                    ...
                    # SHORT: EMA 21 < 50, Price < 200 EMA
                    elif ema_short_signal(hist, i, htf_row):
                        if router.has_open(pair):
                            logging.info("[%s] No-trade (already open)", pair)
                        else:
                            logging.info(
                                "[%s] SHORT TREND signal  ema21=%.2f < ema50=%.2f | price=%.2f < ema200=%.2f | adx=%.1f",
                                pair,
                                bar.ema21,
                                bar.ema50,
                                bar.c,
                        ...
                        logging.info(
                            "[%s] No-trade (no EMA signal)  ema21=%.2f ema50=%.2f ema200=%.2f | adx=%.1f  atr=%.5f",
                            pair,
                            bar.ema21,
                            bar.ema50,
                            bar.ema200,
...
# ────────────────────────────────────────────────────────────────
#  Signal consumer (same as breakout engine)
# ────────────────────────────────────────────────────────────────
async def consume(router: RiskRouter):
    MAX_OPEN = getattr(ema_config, "MAX_OPEN_CONCURRENT", 3)
    MAX_RISK = getattr(ema_config, "MAX_TOTAL_RISK_PCT", 0.30)
    MAX_PER_SIDE = getattr(ema_config, "MAX_PER_SIDE_OPEN", 1)
    MAX_AGE = getattr(ema_config, "MAX_SIGNAL_AGE_SEC", 60)
    COALESCE_SEC = getattr(ema_config, "COALESCE_SEC", 5)
    ...
    while True:
        ...
        if daily_count[today] >= getattr(ema_config, "MAX_TRADES_PER_DAY", 5):
        ...
        busy_clusters = {
            ema_config.CLUSTER.get(p.signal.symbol)
            for p in list(router.book.values()) + list(router.pending.values())
            if ema_config.CLUSTER.get(p.signal.symbol)
        }

        for s in fresh:
            side_norm = "Buy" if s.side.lower() == "buy" else "Sell"
            cid = ema_config.CLUSTER.get(s.symbol)
            ...
            try:
                ...
                logging.info(
                    ...
                    s.symbol,
                    daily_count[today],
                    getattr(ema_config, "MAX_TRADES_PER_DAY", 5),
                    open_used,
                    risk_used * 100,
                )
            except Exception as e:
...
async def main():
    ...
    pairs = getattr(ema_config, "PAIRS_LRS", ["ETHUSDT"])
    
    logging.info("="*80)
    logging.info("EMA TREND FOLLOWING LIVE ENGINE - 1H Timeframe")
    logging.info("Pairs: %s", ", ".join(pairs))
    logging.info("Config: ADX_FLOOR=%d, RR_TARGET=%.1f, REGIME=%s",
                 ema_config.ADX_HARD_FLOOR,
                 ema_config.RR_TARGET,
                 "ON" if getattr(ema_config, "REGIEME_FILTER_ENABLED", False) else "OFF")
    logging.info("="*80)
    ...
    run_command_bot(router, ema_config)
