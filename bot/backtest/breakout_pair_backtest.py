...
from bot.helpers import (
    breakout_config, compute_indicators, build_htf_levels, update_htf_levels_new,
    breakout_short_signal, breakout_long_signal, round_price, align_by_close,
...
def backtest(df: pd.DataFrame,
    ...
    """
    # Warm-up guard
    required_warmup_days = getattr(breakout_config, "HTF_DAYS", 15) + 5  # +5 for safety
    ...
    session_windows = getattr(breakout_config, "SESSION_WINDOWS", {})
    good_hours = set()
    ...
    last_sl_ts = 0.0
    last_tp_ts = 0.0
    cooldown_sl_sec = getattr(breakout_config, "COOLDOWN_DAYS_AFTER_SL", 0.5) * 86400
    cooldown_tp_sec = getattr(breakout_config, "COOLDOWN_DAYS_AFTER_TP", 0.25) * 86400

    # Daily trade cap
    daily_count = defaultdict(int)
    max_daily = getattr(breakout_config, "MAX_TRADES_PER_DAY", 3)
    ...
    for i in range(test_start_idx, len(df)):
        ...
        if pos is not None:
            ...
            if reason:
                ...
                fee = fees_usd(pos.entry, exit_px, pos.qty, breakout_config.FEE_BPS)
                pnl = pnl_gross - fee
        ...
        if pos is None:
            ...
            max_atr_mom = getattr(breakout_config, "NEAR_HTF_MAX_ATR_MOM_BACKTEST", 2.0)
            ...
            adx_hard_floor = getattr(breakout_config, "ADX_HARD_FLOOR", 30)
            ...
            wick_buf = getattr(breakout_config, "WICK_BUFFER", 0.25)
            stop_off = cushion * sl_base + wick_buf * bar.atr
            rr_target = getattr(breakout_config, "RR_TARGET", 1.5)
            ...
            if (bar.k_slow < k_long_max
                and breakout_long_signal(df, i, htf_row)):

                # Entry with execution delay
                if i + 1 < len(df):
                    next_bar = df.iloc[i + 1]
                    entry = float(next_bar.o) * (1 + breakout_config.SLIP_BPS / 10_000)
                else:
                    entry = float(bar.c) * (1 + breakout_config.SLIP_BPS / 10_000)
            ...
            # SHORT BREAKOUT: k_slow must be > 30 (not oversold)
                  ...
                  and breakout_short_signal(df, i, htf_row)):
...
if __name__ == "__main__":
    ...
    # Config
    pair = getattr(breakout_config, "PAIR", "LINKUSDT")
    interval = getattr(breakout_config, "INTERVAL", "15")
    equity = getattr(breakout_config, "STAKE_SIZE_USD", 1000.0)
    risk_pct = getattr(breakout_config, "RISK_PCT", 0.1)
    ...
    days_back = 30
    bars_needed = days_back * 180 # 96 bars per day for 1
